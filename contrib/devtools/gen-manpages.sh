...
LITECOIND=${LITECOIND:-$SRCDIR/theholyrogerd}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/theholyroger-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/theholyroger-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/theholyroger-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
ROGERVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
