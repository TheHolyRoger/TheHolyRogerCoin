The Holy Roger Core version 0.13.2 is now available from:
...
ECDSA signatures inside The Holy Roger transactions now use validation using
...
Previous versions of The Holy Roger Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of The Holy Roger Core was to raise the default minimum
relay fee.

The Holy Roger Core 0.13.2 will have a strict maximum size on the mempool. The
...
The Holy Roger Core 0.13.2 also introduces new default policy limits on the length and
...
The Holy Roger Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. The Holy Roger Core will relay transactions with
...
entirely in The Holy Roger Core 0.13, and it is at this time undecided whether the
...
The Holy Roger Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
The Holy Roger Core automatically creates a hidden service to listen on, without
manual configuration. The Holy Roger Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if The Holy Roger Core is listening, and
...
The Holy Rogerd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals The Holy Roger
Core to use floating fees. By default, The Holy Roger Core will use floating
fees.
...
At all times, The Holy Roger Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ROGER.
Furthermore, The Holy Roger Core will never create transactions paying less than
...
Various code modernizations have been done. The The Holy Roger Core code base has
...
for a new opcode in the The Holy Roger scripting system that in combination with
...
The Holy Roger transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of The Holy Roger Core.
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming The Holy Roger wallet
  software more difficult and which seriously complicates the design of smart
  contracts for The Holy Roger.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each The Holy Roger block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** The Holy Roger addresses (both P2PKH addresses
  ...
  believed to be provided by The Holy Roger's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of The Holy Roger's current ledger (called the UTXO set),
  ...
  note that The Holy Roger Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  The Holy Roger users to individually opt-in to almost any change in the The Holy Roger
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and The Holy Roger Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the The Holy Roger
client to The Holy Roger Core 0.13.2 to signal for segwit support. When a
...
Since The Holy Roger Core 0.10.0, nodes have defaulted to only relaying and
